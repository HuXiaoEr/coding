...
// over一半
public class N32_从1到n整数中1出现的次数 {
  ...
      }
	...
	public static int NumberOf1Between1AndN_Solution(int n) {
		...
		for(int i = 1; i <= n; i*= 10){
			...
			else if((a % 10) == 1) res += a/10*i + b + 1;
			else res += a/10*i;
			/*
			res += (a + 8) / 10 * i;
			res =  (a % 10 == 1) ? res + b + 1 : res;
			*/
		}
		return res;
	}
	...
	public static int NumberOf1Between1AndN_Solution2(int n) {
	    ...
	    int last = n % (int)Math.pow(10, count);
	    if(topDigit == 1) res += last + 1; //最高位1的个数
	    else res += Math.pow(10, count);   //最高位1的个数
	    res += topDigit * count * Math.pow(10, count-1);  //除最高位外1的个数
		return res + NumberOf1Between1AndN_Solution(last);
    }
